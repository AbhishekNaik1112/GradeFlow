...
export async function addDocuments(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!title || !content || !deadline || !userEmail || !type || !status) {
      res.status(400).json({ error: "title, content, type, deadline, userEmail, and status are required." });
      return;
    }
    const embedding = await embedText(`${title} ${content}`);
    const newDoc = new DocumentModel({ title, content, deadline: deadline || null, userEmail, status, type, embedding });
    await newDoc.save();
...
export async function addDocumentsBulk(req: Request, res: Response): Promise<void> {
  try {
    const documents = req.body;
    if (!Array.isArray(documents) || documents.length === 0) {
      res.status(400).json({ error: "A non-empty array of documents is required." });
      return;
    }
    const texts = documents.map(doc => `${doc.title} ${doc.content}`);
    const embeddings = await embedTexts(texts);
    const docsToSave = documents.map((doc, i) => new DocumentModel({
      title: doc.title,
      content: doc.content,
    ...
    res.status(201).json({ message: "Bulk documents added successfully", count: docsToSave.length });
  } catch (error) {
  ...
  try {
    ...
    const scoredDocs = documents.map(doc => {
      if (!doc.embedding || !Array.isArray(doc.embedding)) {
        return { ...doc.toObject(), score: 0, keywordMatch: false, similarityScore: 0 };
      }
      const keywordMatch = doc.title.toLowerCase().includes(lowerQuery) || doc.content.toLowerCase().includes(lowerQuery);
      const similarityScore = cosineSimilarity(queryEmbedding, doc.embedding);
      const finalScore = keywordMatch ? similarityScore * 1.1 : similarityScore;
      return { ...doc.toObject(), score: finalScore, keywordMatch, similarityScore };
    }).filter(doc => doc.keywordMatch || doc.similarityScore >= similarityThreshold);
